...
- `Test2/Testarticle.pdf`；
- 与该 PDF 摘要绑定的 `Test2/review-work/Extractedmd/full.md`；
- 既有本地审稿报告；
- `benchmark-spec.json`。
...
```powershell
python Test2\reproduction\run.py --plan-only `
  --workfolder Test2\reproduction\plan-output
```
...
```powershell
python Test2\reproduction\run.py --mode agent `
  --profile Test2\reproduction\reproduction_profile.toml `
  --workfolder Test2\reproduction\agent-output
```
...
```powershell
python Test2\reproduction\numeric_cli.py `
  --pdf Test2\Testarticle.pdf `
  --markdown Test2\review-work\Extractedmd\full.md `
  --json-out Test2\reproduction\numeric-output\validation-report.json `
  --markdown-out Test2\reproduction\numeric-output\validation-report.md
```
...
```powershell
python -m unittest Test2.tests.test_reproduction -v
python -m unittest discover -s Test2\tests -v
python -m compileall -q Test2\script Test2\reproduction Test2\tests
```
