...
and tests moved to `../archive/clef-sdk-python-0.2/`; the Rust
compiler/scheduler, Protobuf fixtures, and fake adapter contracts remain frozen
`0.2` migration evidence. They are not silently translated and are no longer
the authoritative model for new workflow code.
...
| Contract-only adapter manifests | One-shot `agenstro.plugin/v1` JSONL subprocesses |
...
5. Put provider/effect command arrays in a `clef.runtime/v1` JSON file and set
   `TACTUS_RUNTIME_CONFIG` before invoking the compiled script.
6. Use `parallel` only where the script explicitly wants concurrent execution.
